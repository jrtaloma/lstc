...
# https://github.com/dawnranger/DNFCS-pytorch/blob/master/idec.py
class AE(nn.Module):
...
class DNFCS(nn.Module):
    ...
    def forward(self, x):
        x_rec, z = self.ae(x)
        d1 = compute_similarity(z, self.centroids)
        centroids_mean = torch.mean(self.centroids.data, dim=0)
        d2 = torch.sum((self.centroids - centroids_mean[None, :])**2, dim=1)
        eta = self.beta / d2.sum()
        d2 = eta * d2

        ## Q (batch_size, n_clusters)
        Q = torch.pow(d1 - d2[None, :], -1 / (self.m - 1))
        sum_columns_Q = torch.sum(Q, dim=1).view(-1, 1)
        Q = Q / sum_columns_Q

        ## S_fw : fuzzy within-cluster scatter matrix
        S_fw = (Q**self.m) * d1

        ## S_fb : fuzzy between-cluster scatter matrix
        fb = torch.sum((z - centroids_mean[None, :])**2, dim=1)
        S_fb = eta * (Q**self.m) * fb[:, None]

        return x_rec, z, Q, S_fw, S_fb
