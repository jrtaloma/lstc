...
from net.model import iTransformer, pretrain_ae, predict, pretrain_ae_augmented, predict_augmented
...
if __name__ == '__main__':
    ...
    # Autoencoders
    model = iTransformer(seq_len=args.seq_len, d_model=args.d_model, dropout=args.dropout, factor=args.factor, n_heads=args.n_heads, d_ff=args.d_ff, activation=args.activation, e_layers=args.e_layers, output_attention=args.output_attention, use_norm=args.use_norm)
    model = model.to(device)
    model_augmented = iTransformer(seq_len=args.seq_len, d_model=args.d_model, dropout=args.dropout, factor=args.factor, n_heads=args.n_heads, d_ff=args.d_ff, activation=args.activation, e_layers=args.e_layers, output_attention=args.output_attention, use_norm=args.use_norm)
    ...
    optimizer = torch.optim.Adam(list(model.parameters()) + list(criterion_kmeans.parameters()) + list(model_augmented.parameters()) + list(criterion_kmeans_augmented.parameters()), lr=args.lr)
