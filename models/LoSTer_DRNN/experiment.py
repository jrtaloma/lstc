...
def train_epoch(args, model, model_augmented, train_loader, test_loader, criterion_rec, criterion_kmeans, criterion_kmeans_augmented, criterion_instance_contrastive, criterion_cluster_contrastive, optimizer, scheduler, epoch, device):
    model.train()
    ...
    for inputs, inputs_augmented, _ in tqdm(train_loader, total=len(train_loader), leave=False):
        ...
        loss_total = loss_rec + args.alpha * loss_k_means + loss_instance_contrastive + loss_cluster_contrastive
        loss_total.backward()
        optimizer.step()
...
def train(args, model, model_augmented, train_loader, test_loader, criterion_rec, criterion_kmeans, criterion_kmeans_augmented, criterion_instance_contrastive, criterion_cluster_contrastive, optimizer, scheduler, path_ckpt, device):
    ...
    for epoch in tqdm(range(args.epochs), total=args.epochs, leave=False):
        _, preds, _, ri, ari, nmi, silhouette = train_epoch(args, model, model_augmented, train_loader, test_loader, criterion_rec, criterion_kmeans, criterion_kmeans_augmented, criterion_instance_contrastive, criterion_cluster_contrastive, optimizer, scheduler, epoch, device)
        if ri > best_ri_score:
            best_ri_score = ri
